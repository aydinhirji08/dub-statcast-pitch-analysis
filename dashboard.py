...
def load_data():
    ...
    pitchers = ['Varland, Louis', 'Cole, Gerrit', 'Rodon, Carlos', 'Burnes, Corbin', 'Alcantara, Sandy', 
                'Musgrove, Joe', 'Skenes, Paul', 'Snell, Blake', 'Severino, Luis', 'Nola, Aaron',
                'Freddy, Peralta', 'Cease, Dylan', 'Stroman, Marcus', 'Castillo, Luis', 'Kluber, Corey',
                'Kershaw, Clayton', 'Degrom, Jacob', 'Judge, Aaron', 'Turner, Trea', 'Soto, Juan',
                'Harper, Bryce', 'Acuna, Ronald', 'Trout, Mike', 'Mookie, Betts', 'Yelich, Christian',
                'Freeman, Freddie', 'Tatis, Fernando', 'Machado, Manny', 'Arenado, Nolan', 'Goldschmidt, Paul']
...
if analysis_mode == "Compare Pitchers":
    ...
    if pitcher_1 == pitcher_2:
        ...
        with col1:
            ...
            fig_p1 = px.scatter(p1_sample, x='pfx_x', y='pfx_z', color='pitch_type', title=f"{pitcher_1} Movement", height=500, 
                               labels={'pfx_x': 'Horizontal (in)', 'pfx_z': 'Vertical (in)'}, color_discrete_sequence=px.colors.qualitative.Bold)
        ...
        with col2:
            ...
            fig_p2 = px.scatter(p2_sample, x='pfx_x', y='pfx_z', color='pitch_type', title=f"{pitcher_2} Movement", height=500,
                               labels={'pfx_x': 'Horizontal (in)', 'pfx_z': 'Vertical (in)'}, color_discrete_sequence=px.colors.qualitative.Bold)
    ...
    with col1:
        ...
        fig_mech = px.scatter(sample, x='release_speed', y='release_spin_rate', color='pitch_type', height=500, 
                             labels={'release_speed': 'Velocity (mph)', 'release_spin_rate': 'Spin Rate (RPM)'}, color_discrete_sequence=px.colors.qualitative.Bold)
    ...
    with col2:
        fig_move = px.scatter(filtered_df, x='pfx_x', y='pfx_z', color='pitch_type', height=500, 
                             labels={'pfx_x': 'Horizontal (in)', 'pfx_z': 'Vertical (in)'}, color_discrete_sequence=px.colors.qualitative.Bold)
