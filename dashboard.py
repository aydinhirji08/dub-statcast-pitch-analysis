...
analysis_mode = st.sidebar.radio(
    "Mode",
    ["Single Pitcher", "Compare Pitchers"]
)
    ...
    if pitcher_1 == pitcher_2:
        ...
        with col1:
            vel_data = pd.DataFrame({
                'Pitcher': [pitcher_1, pitcher_2],
                'Velocity': [p1_data['release_speed'].mean(), p2_data['release_speed'].mean()]
            })
            fig_vel = px.bar(vel_data, x='Pitcher', y='Velocity', color='Velocity', 
        ...
        with col2:
            spin_data = pd.DataFrame({
                'Pitcher': [pitcher_1, pitcher_2],
                'Spin Rate': [p1_data['release_spin_rate'].mean(), p2_data['release_spin_rate'].mean()]
            })
            fig_spin = px.bar(spin_data, x='Pitcher', y='Spin Rate', color='Spin Rate',
        ...
        with col1:
            ...
            p1_arsenal = p1_data.groupby('pitch_type').agg({
                'release_speed': 'mean',
        ...
        with col2:
            ...
            p2_arsenal = p2_data.groupby('pitch_type').agg({
                'release_speed': 'mean',
                'release_spin_rate': 'mean',
                'pitch_type': 'count'
            }).round(1)
        ...
        with col1:
            ...
            fig_p1 = px.scatter(p1_sample, x='pfx_x', y='pfx_z', color='pitch_type',
                               title=f"{pitcher_1} Movement", height=500,
        ...
        with col2:
            ...
            fig_p2 = px.scatter(p2_sample, x='pfx_x', y='pfx_z', color='pitch_type',
                               title=f"{pitcher_2} Movement", height=500,
                               labels={'pfx_x': 'Horizontal (in)', 'pfx_z': 'Vertical (in)'},
                               color_discrete_sequence=px.colors.qualitative.Bold)
...
# =========================================================================
    ...
    batter_hand = st.sidebar.radio(
        "Batter",
        ["All", "Right (RHH)", "Left (LHH)"]
    )
    ...
    with col1:
        ...
        k_data = pd.DataFrame({
            'Pitcher': [selected_pitcher, 'League'],
            'K Rate': [pitcher_metrics['k_rate'], league_metrics['k_rate']]
        })
        fig_k = px.bar(k_data, x='Pitcher', y='K Rate', color='K Rate', 
                      color_continuous_scale='Blues', height=400)
    ...
    with col2:
        bb_data = pd.DataFrame({
            'Pitcher': [selected_pitcher, 'League'],
            'BB Rate': [pitcher_metrics['walk_rate'], league_metrics['walk_rate']]
        })
        fig_bb = px.bar(bb_data, x='Pitcher', y='BB Rate', color='BB Rate',
                       color_continuous_scale='Reds', height=400)
        fig_bb.update_layout(template='plotly_dark', paper_bgcolor='#0f1429', plot_bgcolor='#0f1429', showlegend=False)
        st.plotly_chart(fig_bb, use_container_width=True)
    
    with col3:
        whiff_data = pd.DataFrame({
            'Pitcher': [selected_pitcher, 'League'],
            'Whiff Rate': [pitcher_metrics['whiff_rate'], league_metrics['whiff_rate']]
        })
        fig_whiff = px.bar(whiff_data, x='Pitcher', y='Whiff Rate', color='Whiff Rate',
    ...
    arsenal = filtered_df.groupby('pitch_type').agg({
        'release_speed': 'mean',
        'release_spin_rate': 'mean',
        'pitch_type': 'count'
    }).round(1)
    ...
    count_stats = pitcher_all.groupby('count').agg({
    ...
    if len(count_vel) > 0:
        fig_count = px.bar(count_vel, labels={'value': 'Avg Velocity (mph)', 'count': 'Count'},
                           color=count_vel.values, color_continuous_scale='Reds', height=400)
    ...
    with col1:
        ...
        fig_spin = px.bar(spin_by_type, color=spin_by_type.values, 
                         color_continuous_scale='Blues', height=400,
                         labels={'value': 'Spin Rate (RPM)'})
    ...
    with col2:
        ...
        fig_vel = px.bar(vel_by_type, color=vel_by_type.values,
                        color_continuous_scale='Reds', height=400,
                        labels={'value': 'Velocity (mph)'})
    ...
    with col1:
        ...
        fig_mech = px.scatter(sample, x='release_speed', y='release_spin_rate',
                             color='pitch_type', height=500,
                             labels={'release_speed': 'Velocity (mph)', 'release_spin_rate': 'Spin Rate (RPM)'},
                             color_discrete_sequence=px.colors.qualitative.Bold)
    ...
    with col2:
        fig_move = px.scatter(filtered_df, x='pfx_x', y='pfx_z',
                             color='pitch_type', height=500,
                             labels={'pfx_x': 'Horizontal (in)', 'pfx_z': 'Vertical (in)'},
                             color_discrete_sequence=px.colors.qualitative.Bold)
